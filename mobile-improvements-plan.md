...
- Keep the full sidebar for jump-to-date.

**Plan:**
- In `mobile-app.py`, inside the mobile layout block: add a row above the week label with two buttons “Prev week” and “Next week” that update `st.session_state.week_start` and rerun. Style with MOBILE_CSS so it’s full-width and touch-friendly. Keep sidebar as-is for jump.
